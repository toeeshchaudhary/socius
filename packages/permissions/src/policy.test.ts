...
describe("ConfiguredPolicyEngine", () => {
  ...
  test("requires confirmation when any capability needs it", () => {
    expect(engine.evaluate(req({ capabilities: ["fs.read", "fs.write"] })).decision).toBe(
      "confirm",
    );
  });
...
describe("ConfiguredPolicyEngine overrides", () => {
  test("per-tool override loosens a confirm capability to allow", () => {
    const engine = new ConfiguredPolicyEngine(
      { "fs.write": "confirm" },
      { tools: { "git.commit": "allow" } },
    );
    expect(
      engine.evaluate(req({ toolName: "git.commit", capabilities: ["fs.write"] })).decision,
    ).toBe("allow");
    // a different tool is unaffected
    expect(
      engine.evaluate(req({ toolName: "fs.write", capabilities: ["fs.write"] })).decision,
    ).toBe("confirm");
  });
  ...
  test("per-tool override can deny", () => {
    ...
    expect(engine.evaluate(req({ toolName: "fs.read", capabilities: ["fs.read"] })).decision).toBe(
      "deny",
    );
  });
  ...
  test("path deny always wins", () => {
    ...
    expect(
      engine.evaluate(req({ capabilities: ["fs.write"], resources: ["/etc/passwd"], mode: "live" }))
        .decision,
    ).toBe("deny");
  });
  ...
  test("a trusted path prefix downgrades confirm to allow", () => {
    ...
    expect(
      engine.evaluate(
        req({ capabilities: ["fs.write"], resources: ["/home/me/scratch/note.txt"], mode: "live" }),
      ).decision,
    ).toBe("allow");
    // outside the trusted prefix, still confirm
    expect(
      engine.evaluate(
        req({ capabilities: ["fs.write"], resources: ["/home/me/other.txt"], mode: "live" }),
      ).decision,
    ).toBe("confirm");
  });
});
