...
const prompt =
  process.argv.slice(2).join(" ") || "Reply in one short sentence confirming you are online.";
...
process.stderr.write(
  `[e2e] handshake ok: protocol=${hs.protocolVersion} model=${hs.modelId} ready=${hs.modelReady}\n`,
);
...
process.stderr.write(
  `[e2e] --- done: ${tokens} tokens in ${((Date.now() - t1) / 1000).toFixed(1)}s ---\n`,
);
